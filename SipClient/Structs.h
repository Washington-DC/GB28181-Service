...
struct SessionInfo {
    ...
    EnumPlayMode PlayMode = EnumPlayMode::Realtime;
    ...
    float DownloadSpeed = 1.0; // 回放或下载时，有此字段

    std::shared_ptr<ChannelInfo> Channel = nullptr; //对应的通道信息

    // 视频回放时使用起始时间和结束时间作为流的地址，不再使用ssrc作为地址，避免多个系统之间，不同ssrc表示的不同的流
    std::string PlaybackStream() { return fmt::format("{}_{}", StartTime, EndTime); }
