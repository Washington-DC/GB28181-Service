...
bool SipDevice::Init() {
    ...
    HttpServer::GetInstance()->AddStreamChangedCallback(
        std::bind(&SipDevice::OnStreamChangedCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    ...
    if (event->response->status_code == 401 || event->response->status_code == 403) {
        ...
        if (www_authenticate_header) {
            ...
            eXosip_add_authentication_info(
                _sip_context, this->ID.c_str(), this->ID.c_str(), _sip_server_info->Password.c_str(), "md5", www_authenticate_header->realm);
        }
    }
}
...
void SipDevice::OnCallACK(eXosip_event_t *event) {
    ...
    if (iter != _session_map.end()) {
        if (iter->second->Used == false) {
            ...
            if (iter->second->PlayMode == EnumPlayMode::Realtime) {
                iter->second->Start();
...
void SipDevice::OnCallInvite(eXosip_event_t *event) {
    ...
    session->PlayMode = EnumPlayMode::Realtime;

    //判断是回放还是实时
    if (strstr(sdp_body->body, "s=Playback")) {
        session->PlayMode = EnumPlayMode::Playback;
        std::string text = sdp_body->body;
        ParseTimeStr(text, session->StartTime, session->EndTime);

        auto file = DbManager::GetInstance()->QueryOne(channel_info->DbName(), session->StartTime, session->EndTime);
        if (file == nullptr) {
            SendInviteResponse(event, "", 404);
            return;
        } else {
    ...
    if (strstr(sdp_body->body, "s=Download")) {
        session->PlayMode = EnumPlayMode::Download;
        std::string text = sdp_body->body;
        ParseTimeStr(text, session->StartTime, session->EndTime);

        std::string speed = ParseDownloadSpeed(sdp_body->body);
        session->DownloadSpeed = std::stof(speed);
    ...
    {
        auto iter = std::find_if(_session_map.begin(), _session_map.end(), [session](const std::pair<int32_t, std::shared_ptr<Session>> &s) {
            return s.second->TargetIP == session->TargetIP and s.second->TargetPort == session->TargetPort and s.second->Channel == session->Channel;
        });
    ...
    auto info = fmt::format(
        ss.str(), invite_video_channel_id, this->IP, g_PlayMode[(uint32_t)session->PlayMode], this->IP, session->LocalPort,
        session->UseTcp ? "TCP/RTP/AVP" : "RTP/AVP", ssrc);

    if (session->PlayMode == EnumPlayMode::Download) {
        info.append(fmt::format("a={}\r\n", file_size));
    }
...
void SipDevice::MobilePositionTask() {
    while (_is_running && _register_success && _subscription_dialog_id > 0) {
        ...
        if (OSIP_SUCCESS
            != eXosip_insubscription_build_notify(_sip_context, _subscription_dialog_id, EXOSIP_SUBCRSTATE_PENDING, EXOSIP_NOTIFY_PENDING, &notify_message)) {
...
std::string SipDevice::GenerateBasicParamXML(const std::string &sn) {
    ...
    return fmt::format(
        text, sn, this->ID, this->Name, this->ID, _sip_server_info->ID, _sip_server_info->IP, _sip_server_info->Port, _sip_server_domain,
...
void SipDevice::SendRecordInfo(const std::string &sn, const std::string &channel_id, const std::string &start_time, const std::string &end_time) {
    ...
    auto vec_files = DbManager::GetInstance()->Query(channel_info->DbName(), start, end);
    if (vec_files.empty()) {
        send_empty_response();
        return;
    }
...
std::string SipDevice::ParseDownloadSpeed(const std::string &text) {
    auto tokens = nbase::StringTokenize(text.c_str(), "\n");
    for (auto &&token : tokens) {
        if (strstr(token.c_str(), "downloadspeed") || strstr(token.c_str(), "downloadspeed")) {
            auto result = token.substr(16);
            result = nbase::StringTrim(result);
            SPDLOG_INFO("---DownloadSpeed: {}", result);
            return result;
        }
    }
...
bool SipDevice::ParseTimeStr(std::string &text, int64_t &start_time, int64_t &end_time) {
    ...
    for (auto &&token : tokens) {
        if (strstr(token.c_str(), "t=")) {
            ...
            return nbase::StringToInt64(start, &start_time) && nbase::StringToInt64(end, &end_time);
        }
    }
    return false;
}
...
void SipDevice::OnStreamChangedCallback(const std::string &app, const std::string &stream, bool regist) {
    ...
    std::cout << "----------------------->>>   " << _session_map.size() << std::endl;
    //只需要判断录像回放的流注销事件即可
...
void Session::Start() {
    if (this->PlayMode == EnumPlayMode::Realtime) {
        // 实时流：向流媒体服务器发送实时推送请求，需要指定收发数据的端口，SSRC和时间范围
        auto ret = HttpClient::GetInstance()->StartSendRtp(
            this->Channel->App, this->Channel->Stream, this->SSRC, this->TargetIP, this->TargetPort, this->LocalPort, this->UseTcp);
    } else {

        this->Speed(this->DownloadSpeed);
        // 录像回放：先加载MP4文件，然后sendRTP
        auto ret = HttpClient::GetInstance()->StartSendRtp(
            "record", this->PlaybackStream(), this->SSRC, this->TargetIP, this->TargetPort, this->LocalPort, this->UseTcp);
    }
}
...
void Session::LoadMP4File() {
    if (this->PlayMode != EnumPlayMode::Realtime) {
        // 录像回放：先加载MP4文件，然后sendRTP
        auto ret = HttpClient::GetInstance()->LoadMP4File("record", this->PlaybackStream(), this->FilePath);
    }
}
...
void Session::Stop() {
    HttpClient::GetInstance()->StopSendRtp(
        this->PlayMode == EnumPlayMode::Realtime ? this->Channel->App : "record",
        this->PlayMode == EnumPlayMode::Realtime ? this->Channel->Stream : this->PlaybackStream());
}

// 向流媒体服务器发送暂停请求
void Session::Pause(bool flag) {
    if (_paused != flag) {
        auto ret = HttpClient::GetInstance()->SetPause("record", this->PlaybackStream(), flag);
        _paused = flag;
    }
}
...
void Session::Speed(float speed) {
    auto ret = HttpClient::GetInstance()->SetSpeed("record", this->PlaybackStream(), speed);
}
